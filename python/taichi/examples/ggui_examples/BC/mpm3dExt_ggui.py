...
def substep(g_x: float, g_y: float, g_z: float):
    ...
    for p in F_x:
        ...
        if F_materials[p] == JELLY:  # jelly, make it softer
            h = 0.3
        elif F_materials[p] == CONCRETE:  # concrete, stiffer
            h = 4.0
        ...
        for d in ti.static(range(3)):
            ...
            elif F_materials[p] == CONCRETE:  # Concrete behaves rigidly with tiny plasticity
                new_sig = ti.min(ti.max(sig[d, d], 1 - 5e-3), 1 + 1e-3)
        ...
        if F_materials[p] == WATER:
            ...
            new_F[0, 0] = J
            F_dg[p] = new_F
        elif F_materials[p] == SNOW or F_materials[p] == CONCRETE:
